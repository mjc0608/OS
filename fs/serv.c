...
serve_read(envid_t envid, union Fsipc *ipc)
{
    ...
    r = file_read(o->o_file, ret->ret_buf,
            req->req_n<PGSIZE?req->req_n:PGSIZE, o->o_fd->fd_offset);
    if (r < 0) return r;
...
serve_write(envid_t envid, struct Fsreq_write *req)
{
    ...
    int actual_write_size = req->req_n;
    if (actual_write_size > PGSIZE - (sizeof(int) + sizeof(size_t))) {
        actual_write_size = PGSIZE - (sizeof(int) + sizeof(size_t));
    }
    r = file_write(o->o_file, req->req_buf, actual_write_size, o->o_fd->fd_offset);
    if (r < 0) return r;
