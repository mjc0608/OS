...
boot_alloc(uint32_t n)
{
	...
	if (n < 0) {
		panic("memory size invalid\n");
	}

	result = nextfree;

	if (PGNUM(PGNUM(result)) >= npages) {
		panic("invalid nextfree: 0x%x\n", nextfree);
	}
...
page_free(struct Page *pp)
{
	...
	if (pp->pp_ref != 0) {
		panic("page_free is called when pp_ref is not 0\n");
	}
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	if (!pgdir) {
		panic("pgdir_walk with a NULL pgdir\n");
	}
